...
void Topology::calculateWeightedSingleShortestPathsTo(Node *_target) const
{
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < dest->getNumInLinks(); i++) {
            ...
            Node *src = dest->getLinkIn(i)->getLinkInRemoteNode();//EV<<"calculate path from "<<src->getModule()->getFullPath()<<" to "<<dest->getModule()->getFullPath()<<", gate = "<<dest->getLinkIn(i)->getLinkInRemoteGate()<<endl;
            if (!src->isEnabled())
                continue;

            double linkWeight = dest->getLinkIn(i)->getWeight();//EV<<"linkweight = "<<linkWeight<<endl;
            ...
            if (newdist != INFINITY && src->dist >= newdist) { // it's a valid shorter path from src to target node modified by HiNA;
                ...
                q.insert(it, src);//EV<<"it's a shorter path, insert src to "<<q.size()<<endl;
            }
//            else if (!contains(src->outPaths, dest->inLinks[i]))modified by HiNA;
//                src->outPaths.push_back(dest->inLinks[i]);modified by HiNA;
        }
    }
}
