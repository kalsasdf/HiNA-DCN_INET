...
void HPCC::initialize(int stage)
{
    if (stage == INITSTAGE_LOCAL){
        ...
        currentRateVector.setName("currentRate (bps)");
        send_windowVector.setName("send_window (byte)");
...
void HPCC::receiveAck(Packet *pck)
{
    ...
    currentRate = send_window *8/ baseRTT.dbl();
    EV<<"the ACK computed rate is "<<currentRate<<endl;
    currentRateVector.recordWithTimestamp(simTime(),currentRate);
    send_windowVector.recordWithTimestamp(simTime(),send_window);
