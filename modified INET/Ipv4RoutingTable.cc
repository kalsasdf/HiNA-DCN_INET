...
void Ipv4RoutingTable::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        flowlevel = par("flowlevel");

        WATCH_PTRVECTOR(routes);
//        WATCH_PTRVECTOR(ecmproutes);
    ...
    for (auto e : routes) {
        if (e->isValid()) {
            if (Ipv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask())) { // match
                bestRoute = const_cast<Ipv4Route *>(e);EV_INFO<<"findBestMatchingRoute(), route = "<<bestRoute->str()<<endl;
                break;
            }
        }
    }
...
//added by HiNA
//Ipv4Route *Ipv4RoutingTable::findreverseRoute(const Ipv4Address& src, const Ipv4Address& dest, Packet *packet) const
//{
//    Enter_Method("findreverseRoute(%u.%u.%u.%u)", dest.getDByte(0), dest.getDByte(1), dest.getDByte(2), dest.getDByte(3));
//    EV<<endl<<"findreverseRoute(), find the reverse path of dest = "<<dest<<endl;
//
//    int packetid=-1;
//    for (auto& region : packet->peekData()->getAllTags<HiTag>()){
//        packetid = region.getTag()->getPacketId();
//        EV << "this_packet_id = "<<packetid<<endl;
//    }
//
//
//}

Ipv4Route *Ipv4RoutingTable::findecmpRoute(const uint32_t IFid_in, const Ipv4Address& src, const Ipv4Address& dest, Packet *packet) const
{
    Enter_Method("findEcmpRoute(%u.%u.%u.%u)", dest.getDByte(0), dest.getDByte(1), dest.getDByte(2), dest.getDByte(3));
    size_t hash_value;
    int flowid;
    int packetid=-1;
    bool reverse=false;
    for (auto& region : packet->peekData()->getAllTags<HiTag>()){
        ...
        packetid = region.getTag()->getPacketId();
        reverse = region.getTag()->getReverse();
        EV << "this_flow_id = "<<flowid<<", this_packet_id = "<<packetid<<endl;
    }
    if(reverse){
        auto it = reverseIFCache.find(packetid);
        if (it != reverseIFCache.end())
        {
            Ipv4Route *bestroute = findBestMatchingRoute(dest);
            uint32_t reverse_IF = it->second;
            bestroute->setInterface(ift->getInterfaceById(reverse_IF));
            EV<<"findreverseRoute, find route in reverseIFCache, reverse IF = "<<ift->getInterfaceById(reverse_IF)->getInterfaceFullPath()<< endl;
            return bestroute;
        }
        else
        {
            EV<<"findreverseRoute, not find, return nullptr"<<endl;
            return nullptr;
        }
    }else{
        if(flowlevel){
            hash_value = hashAddresses((unsigned int)src.getInt(),(unsigned int)flowid,(unsigned int)dest.getInt());
            EV<<"findecmpRoute() flowlevel, the hash value of packet = "<< hash_value <<endl;
            Ipv4Route *bestRoute = nullptr;
            for (auto e : routes) {
                if (e->isValid()) {
                    if (Ipv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask()))
                    {
                        EV<<" number of equal interfaces is = "<<ecmpIFs.find(e->getDestination())->second.size()<<endl;
                        if (ecmpIFs.find(e->getDestination())->second.size() == 1)
                        {
                            bestRoute = const_cast<Ipv4Route *>(e);
                            EV_INFO << " route to interface = "<< bestRoute->getInterface()->getInterfaceFullPath() <<endl;
                            break;
                        }
                        else
                        {
                            uint32_t IFid = hash_value % ecmpIFs.find(e->getDestination())->second.size();
                            EV_INFO << " the mod = "<< IFid <<", interface = "<<ecmpIFs.find(e->getDestination())->second[IFid]->getInterface()->getInterfaceFullPath()<<endl;
                            bestRoute=ecmpIFs.find(e->getDestination())->second[IFid];
                            break;
                        }
                    }
                }
            }
            EV<<"findecmpRoute(), packetid "<< packetid <<" reverse interface has been set = "<< bestRoute->getInterface()->getInterfaceFullPath() <<endl;
            reverseIFCache[packetid] = IFid_in;
            return bestRoute;
        }else{
            hash_value = hashAddresses((unsigned int)src.getInt(),(unsigned int)flowid,(unsigned int)packetid,(unsigned int)dest.getInt());
            EV<<"findecmpRoute() packetlevel, the hash value of packet = "<< hash_value <<endl;
            Ipv4Route *bestRoute = nullptr;
            for (auto e : routes) {
                if (e->isValid()) {
                    if (Ipv4Address::maskedAddrAreEqual(dest, e->getDestination(), e->getNetmask()))
            ...
            EV<<"findecmpRoute(), pakcetid "<< packetid <<" reverse interface has been set = "<< bestRoute->getInterface()->getInterfaceFullPath() <<endl;
            reverseIFCache[packetid] = IFid_in;
            return bestRoute;
        }
    }
...
size_t Ipv4RoutingTable::hashAddresses(const unsigned int src, const uint32_t flowid, const uint32_t pckid, const unsigned int dest)
{
    size_t value1 = std::hash<unsigned int>{}(src/23);
    size_t value2 = std::hash<unsigned int>{}(dest/29);
    size_t value3 = std::hash<unsigned int>{}(flowid);
    size_t value4 = std::hash<unsigned int>{}(pckid);
    return std::hash<unsigned int>{}((value1+value2+value3+value4));
}

size_t Ipv4RoutingTable::hashAddresses(const unsigned int src, const uint32_t flowid, const unsigned int dest)
{
    size_t value1 = std::hash<unsigned int>{}(src/23);
    size_t value2 = std::hash<unsigned int>{}(dest/29);
    size_t value3 = std::hash<unsigned int>{}(flowid);
    return std::hash<unsigned int>{}((value1+value2+value3));
}
//added by HiNA
...
void Ipv4RoutingTable::internalAddRoute(Ipv4Route *entry)
{
    ...
    ecmpIFs[entry->getDestination()].push_back(entry);//added by HiNA
