...
void INTHeader::copy(const INTHeader& other)
{
    ...
    delete [] this->hopInfs;
    this->hopInfs = (other.hopInfs_arraysize==0) ? nullptr : new hopInf[other.hopInfs_arraysize];
    hopInfs_arraysize = other.hopInfs_arraysize;
    for (size_t i = 0; i < hopInfs_arraysize; i++) {
...
void INTHeader::parsimPack(omnetpp::cCommBuffer *b) const
{
    ...
    b->pack(hopInfs_arraysize);
    doParsimArrayPacking(b,this->hopInfs,hopInfs_arraysize);
}
...
void INTHeader::parsimUnpack(omnetpp::cCommBuffer *b)
{
    ...
    delete [] this->hopInfs;
    b->unpack(hopInfs_arraysize);
    if (hopInfs_arraysize == 0) {
        this->hopInfs = nullptr;
    } else {
        this->hopInfs = new hopInf[hopInfs_arraysize];
        doParsimArrayUnpacking(b,this->hopInfs,hopInfs_arraysize);
    }
}
...
{
    return hopInfs_arraysize;
}

const hopInf& INTHeader::getHopInfs(size_t k) const
{
    if (k >= hopInfs_arraysize) throw omnetpp::cRuntimeError("Array of size %lu indexed by %lu", (unsigned long)hopInfs_arraysize, (unsigned long)k);
    return this->hopInfs[k];
}

void INTHeader::setHopInfsArraySize(size_t newSize)
{
    handleChange();
    hopInf *hopInfs2 = (newSize==0) ? nullptr : new hopInf[newSize];
    size_t minSize = hopInfs_arraysize < newSize ? hopInfs_arraysize : newSize;
    for (size_t i = 0; i < minSize; i++)
        hopInfs2[i] = this->hopInfs[i];
    delete [] this->hopInfs;
    this->hopInfs = hopInfs2;
    hopInfs_arraysize = newSize;
}
...
void INTHeader::insertHopInfs(size_t k, const hopInf& hopInfs)
{
    if (k > hopInfs_arraysize) throw omnetpp::cRuntimeError("Array of size %lu indexed by %lu", (unsigned long)hopInfs_arraysize, (unsigned long)k);
    handleChange();
    size_t newSize = hopInfs_arraysize + 1;
    hopInf *hopInfs2 = new hopInf[newSize];
    size_t i;
    for (i = 0; i < k; i++)
        hopInfs2[i] = this->hopInfs[i];
    hopInfs2[k] = hopInfs;
    for (i = k + 1; i < newSize; i++)
        hopInfs2[i] = this->hopInfs[i-1];
    delete [] this->hopInfs;
    this->hopInfs = hopInfs2;
    hopInfs_arraysize = newSize;
}

void INTHeader::appendHopInfs(const hopInf& hopInfs)
{
    insertHopInfs(hopInfs_arraysize, hopInfs);
}

void INTHeader::eraseHopInfs(size_t k)
{
    if (k >= hopInfs_arraysize) throw omnetpp::cRuntimeError("Array of size %lu indexed by %lu", (unsigned long)hopInfs_arraysize, (unsigned long)k);
    handleChange();
    size_t newSize = hopInfs_arraysize - 1;
...
unsigned int INTHeaderDescriptor::getFieldTypeFlags(int field) const
{
    ...
    static unsigned int fieldTypeFlags[] = {
        ...
        FD_ISARRAY | FD_ISCOMPOUND | FD_ISRESIZABLE,    // FIELD_hopInfs
    };
...
int INTHeaderDescriptor::getFieldArraySize(omnetpp::any_ptr object, int field) const
{
    ...
    switch (field) {
        case FIELD_hopInfs: return pp->getHopInfsArraySize();
        default: return 0;
    }
}
...
void INTHeaderDescriptor::setFieldArraySize(omnetpp::any_ptr object, int field, int size) const
{
    ...
    INTHeader *pp = omnetpp::fromAnyPtr<INTHeader>(object); (void)pp;
    switch (field) {
        case FIELD_hopInfs: pp->setHopInfsArraySize(size); break;
