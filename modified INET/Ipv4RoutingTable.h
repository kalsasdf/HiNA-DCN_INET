...
#include "inet/HiNA/Messages/HiTag/HiTag_m.h"//added by HiNA
...
class INET_API Ipv4RoutingTable : public cSimpleModule, public IIpv4RoutingTable, protected cListener, public ILifecycle
{
  protected:
    ...
    typedef std::vector<Ipv4Route *> ecmpRouteVector;
    typedef std::map<Ipv4Address, ecmpRouteVector> ecmprouteIFs;
    mutable ecmprouteIFs ecmpIFs;

    typedef std::map<long, int> reverseIFs;
    mutable reverseIFs reverseIFCache;
    //added by HiNA
  ...
  public:
    ...
    //added by HiNA
//    virtual Ipv4Route *findreverseRoute(const Ipv4Address& src,const Ipv4Address& dest, Packet *packet) const override;

    virtual Ipv4Route *findecmpRoute(const uint32_t IFid_in,const Ipv4Address& src,const Ipv4Address& dest, Packet *packet) const override;

    static size_t hashAddresses(const unsigned int src, const uint32_t flowid, const uint32_t pckid, const unsigned int dest);

    static size_t hashAddresses(const unsigned int src, const uint32_t flowid, const unsigned int dest);
    //added by HiNA
