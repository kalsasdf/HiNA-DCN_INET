...
void Ipv4NetworkConfigurator::computeConfiguration()
{
    ...
    if (addStaticRoutesParameter) {
        ...
        else {EV_INFO<<"Ipv4NetworkConfigurator.cc: line 99"<<endl;
...
{EV_INFO<<"Ipv4NetworkConfigurator::configureInterface(NetworkInterface *networkInterface)"<<endl;
    ensureConfigurationComputed(topology);
...
{EV_INFO<<"Ipv4NetworkConfigurator::getInterfaceIpv4Address(L3Address& ret, NetworkInterface *networkInterface, bool netmask)"<<endl;
    auto it = topology.interfaceInfos.find(networkInterface->getId());
    if (it == topology.interfaceInfos.end())
        return false;
    else {
